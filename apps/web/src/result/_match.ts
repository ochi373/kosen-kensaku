	...
	describe("match", () => {
		...
		describe("エリア検索（OR検索）", () => {
			it("エリア1つ指定：そのエリアのみの高専を返す", () => {
				...
				const expectedCount = KOSEN_LIST.filter(
					(k) => k.area === firstArea,
				).length;
		...
		describe("学科検索（OR検索）", () => {
			it("学科1つ指定：その学科を持つ高専を返す", () => {
				const targetSubject = KOSEN_LIST.find((k) => k.subjects.length > 0)
					?.subjects[0];
				if (targetSubject) {
			...
			it("学科複数指定：いずれかの学科を持つ高専を返す（OR検索）", () => {
				const subject1 = KOSEN_LIST.find((k) => k.subjects.length > 0)
					?.subjects[0];
				const subject2 = KOSEN_LIST.find((k) => k.subjects.length > 1)
					?.subjects[1];
				if (subject1 && subject2 && subject1 !== subject2) {
					const result = match([], [subject1, subject2], []);
					expect(result.length).toBeGreaterThan(0);
					result.forEach((r) => {
						const hasMatch = r.subjects.some((s) =>
							[subject1, subject2].includes(s),
						);
		...
		describe("こだわりポイント検索（AND検索）", () => {
			it("こだわり1つ指定：そのこだわりを持つ高専を返す", () => {
				const targetOther = KOSEN_LIST.find((k) => k.others.length > 0)
					?.others[0];
				if (targetOther) {
		...
		describe("matchedList の正確性", () => {
			...
			it("複数マッチ時：matchedList は重複なく全てのマッチを含む", () => {
				...
				if (target && target.subjects.length >= 2) {
					...
					result.forEach((r) => {
						const expectedMatches = r.subjects.filter((s) =>
							[s1, s2].includes(s),
						);
...
const intersect = <T extends string | number>(
	a: readonly T[],
	b: readonly T[],
) => {
	const set = new Set(a);
